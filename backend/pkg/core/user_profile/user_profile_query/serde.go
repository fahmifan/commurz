...
func ListFromUserPkg(users []user_profile.User) []*commurzv1.User {
	return lo.Map(users, func(user user_profile.User, _ int) *commurzv1.User {
		return FromUserPkg(user)
	})
}

func FromUserPkg(user user_profile.User) *commurzv1.User {
	return &commurzv1.User{
		Id:    user.ID.String(),
		Email: user.Email,
